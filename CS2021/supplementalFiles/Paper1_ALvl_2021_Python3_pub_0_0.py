...
def RangerPiece(Piece):
  def __init__(self, Player1):
    super.__init__(self, Player1)
    self._PieceType = "R"

  def CheckMoveIsValid(self, StartTerrain, EndTerrain, DistanceBetweenTiles):
    #check if target is a forest 
    #check if current tile is a forest
    #allow move to tile if so
    #reduce fuel by 1
    if DistanceBetweenTiles == 1:
      if StartTerrain == "~" or EndTerrain == "~":
        return self._FuelCostOfMove * 2
...
class HexGrid:
  ...
  # 15 - new method in HexGrid class
  def GetFogOfWar(self, TileIndex):
    # 15 - return False if tile if any piece is within two cells of that tile index
    for IterTile in self._Tiles:
      if IterTile._x == self._Tiles[TileIndex]._x and IterTile._y == self._Tiles[TileIndex]._y and IterTile._z == self._Tiles[TileIndex]._z:
        pass
      else:
        if max(max(abs(self._Tiles[TileIndex]._x - IterTile._x), abs(self._Tiles[TileIndex]._y - IterTile._y)), abs(self._Tiles[TileIndex]._z - IterTile._z)) <= 2:
          return False
    # 15 - return True otherwise
    return True
  ...
  def ExecuteCommand(self, Items, FuelAvailable, LumberAvailable, PiecesInSupply):
    ...
    if Items[0] == "move":
      ...
      # 14 - check if we have lumber, if not return string "Cannot burn lumber"
      if LumberAvailable > 0:
        # 14 - reduce lumber by random number between 1 and avaliable lumber.
        # Add that same amount of fuel.
        randomNumber = random.randint(1, LumberAvailable)
        LumberChange -= randomNumber
        FuelChange += randomNumber 
        print("Command executed")
      else:
        print("Cannot burn lumber")
  ...
  def DestroyPiecesAndCountVPs(self):
    ...
    for T in self._Tiles:
      if T.GetPieceInTile() is not None:
        ...
        NoOfConnections = 0
        if isinstance(T.GetPieceInTile(),LESSPiece):
          if T.GetPieceInTile().GetBelongsToPlayer1:
            Player1VPs += 1
          else:
            Player2VPs += 1
        for N in ListOfNeighbours:
...
def CheckCommandIsValid(Items):
  if len(Items) > 0:
    ...
    # 14 - return true if command was 'burn'
    elif Items[0] == "burn":
      return True
  return False
